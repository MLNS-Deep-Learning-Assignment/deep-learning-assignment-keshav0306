...
class ResNetEncoder(nn.Module):
    def __init__(self, backbone='resnet18', pretrained=True, num_images=1, init_ch=3):
        ...
        # resnet - conv1 -> bn1 > relu -> maxpool -> layer2 > layer3 > layer4 > avgpool > fc
...
class SumMNISTModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.backbone = ResNetEncoder(config.backbone, init_ch=3)
        ...
        # self.conv2 = self.config.patch_size
        # self.conv1 = nn.Conv2d(1, 32, kernel_size=3, padding=1)
        # self.conv2 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        # self.conv3 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        # self.mid_linear = nn.Linear(128, 128)
        self.linear = nn.Linear(2048, 37)
    
    def forward(self, x):
        # x = self.backbone(x * 255)[4]
        x = self.backbone(x*255)[4]
    ...
    def compute_loss(self, batch):
        ...
        pred = self(img[:, None].repeat(1, 3, 1, 1))
